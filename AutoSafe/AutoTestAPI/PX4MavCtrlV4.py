...
class PX4MavCtrler:
    ...
    def __init__(self, port=20100,ip='127.0.0.1'):
        ...
        self.trueGpsUeCenter=[40.1540302,116.2593683,0]
        self.vibration = 0
                    ...
                    if len(buf)==120:
                        ...
                        if iValue[0] == 1234567897:
                            ...
                            if not isCopterExist:
                                ...
                                self.inSilVect = self.inSilVect +  [copy.deepcopy(vsr)] 
                        ...
                        if(iValue[0]==1234567897):
                            ...
                            for i in range(len(self.inReqVect)):
                                if self.inReqVect[i].copterID == iValue[1]:
                                    ...
                                    break
                            if not isCopterExist:
                                self.inReqVect = self.inReqVect +  [copy.deepcopy(vsr)] 
    ...
    def StartTimeStmplisten(self,cpID=0):
        ...
        """
        self.udp_time = socket.socket(socket.AF_INET, socket.SOCK_DGRAM)  
            ...
            while True:
                ...
                if self.UDPMode>1.5: # If Use MAVLink Mode
                    msg = self.the_connection.recv_match(
                        type=['ATTITUDE', 'LOCAL_POSITION_NED','HEARTBEAT','HOME_POSITION','GLOBAL_POSITION_INT','HIL_ACTUATOR_CONTROLS','STATUSTEXT','HIGHRES_IMU','VIBRATION'],
                        blocking=False)
                    if msg is not None:
                        ...
                        if msg.get_type() == "LOCAL_POSITION_NED":
                            ...
                            if not (abs(self.uavPosGPSHome[0])<1 and abs(self.uavPosGPSHome[1])<1):
                                self.uavGlobalPos[0]=(self.uavPosGPSHome[0]-self.trueGpsUeCenter[0])/180*math.pi*6362000+self.uavPosNED[0]
                                self.uavGlobalPos[1]=(self.uavPosGPSHome[1]-self.trueGpsUeCenter[1])/180*math.pi*4.8823e6+self.uavPosNED[1]
                                self.uavGlobalPos[2]=-(self.uavPosGPSHome[2]-self.trueGpsUeCenter[2])+self.uavPosNED[2]

                        if msg.get_type() == "VIBRATION":
                            self.vibration = msg.vibration_x

                        if msg.get_type() == "HIGHRES_IMU":
                            self.trueAccB[0] = msg.xacc
                            self.truegyro = msg.xgyro
                            self.truemag = msg.xmag
